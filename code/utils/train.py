...
def train_epoch(model, optimizer, criterion, num_batch, sampler, device, log=None, contrastive_w=None):
    model.train()
    total_loss = 0.0
    for step in range(num_batch):
        if contrastive_w is not None:  # 如果contrastive不为None，就进行对比学习
            ...
            cl_loss = model.cl_loss(aug_emb1.view(batch_size, -1), aug_emb2.view(batch_size, -1)) * contrastive_w
        else:
        ...
        if contrastive_w is not None:  # 如果contrastive不为None，就将对比学习的损失加入到总损失中
            loss += cl_loss
        loss.backward()
        optimizer.step()
