SASRec是属于序列推荐里面的经典模型，由于之前并没有怎么接触过推荐系统，所以就想要尝试复现论文并进行一些实验加深对序列推荐任务的理解。
当前仓库主要借鉴以下仓库：
...
### 2025.4.30
今天实验换用不同的对比学习损失，并且尝试使用不同的对比学习方式。
同时，我发现电影实际上是有一些描述的，我觉得只是空白的使用id序列去学习似乎不太能利用这个tag的信息，那么在这里尝试将item_embedding中增加一些描述tag。

这里简单的将所有电影的type给处理了一下，一共有18种电影类型。然后思考一下如何结合这个信息，首先，想要结合这个信息的目的其实是：id和这些属性描述的都是同一个物品，我希望结合信息后可以得到满足两个属性的嵌入：1、因为id的唯一性，所以不同嵌入之间距离比较大。2、因为属性相近，所以我希望属性相近的嵌入之间距离较小。那么或许通过对比学习结合这个信息？
暂时先设计一下几种方案：
- [x] 直接用一个线性层做映射，然后加到embedding里面，但是又担心这个属性太强了直接把id给覆盖了，那么试着用一个门控来解决试试看
这个方案通过实验发现效果变差了很多，但是感觉其实是很奇怪的，因为这个方案只是将属性映射到了embedding里面通过加权求和增加额外信息，当权重为0时其实就是之前的方案，也就不是这样的效果了。暂时想到了几个分析的点可以看一下：
- [ ] 查看保存模型的加权求和的权重
- [ ] 尝试查看embedding的特征分布
...
今天主要完成两个任务：
1. 完成之前没有完成的实验分析
2. 在面试说明中提及加入时间步的优化，这个有一篇相关论文提及了相关的方式，目前先自己尝试用自己的想法实现一个版本，后续再进行复现

在进行实验时发现一个并不合理的现象，当不增加额外的tag的embedding时运行的性能也下降了，那么可能是之前更改模型结构上出现的问题，之前主要修改数据集相关的代码和模型相关的代码，现在检查两个代码是否存在问题。

- [x] 数据集文件：当输入的额外数据为none时，正确输出未改动之前的期望输出，当输入额外数据时，输出结果也是正确的
- [x] 模型文件：发现在前向传播计算mask的过程中，由于之前为了灵活适应不同的输入，将原先的函数拆分成了多个小模块，而原先计算mask的输入在进入forward前就已经经过了np处理，在更改后的版本就没有这一步，所以这里首先做一个修改，同时在训练代码中也做对应修改

这里得到一个教训，对于模型训练过程中的输入数据的修改一定要跟随数据流进行修改，特别是像这里遇到的，针对输入类型的变化，部分处理可能并不会发生报错，但是其处理可能就已经不是预想中的处理了。
待检查项：1. 输入数据的类型，设备，形状。2.输出数据的类型，设备，形状。3.标签数据的相关属性。4.对于运算着重检查关系运算的输入数据和输出数据是否符合预期。

现在重新进行实验，如果发现效果没有提升甚至下降的话再按照之前的方案进行检查。

同时先把时间步这个数据的加载的相关函数实现了。

### 2025.5.7
今天将之前的实验完成了，发现结果确实是不如没有改进之前，并且加入的嵌入的加权和权重逐渐从0.5到0.8，说明原先序列的嵌入还是更为重要，增加的信息似乎起到了反作用，后续需要找方法可视化一下特征向量去验证一下。
然后今天尝试了添加时间步的改进，有一篇论文就是在SASRec基础上加入了时间步信息然后得到了很好的性能，文献的方法是将时间差和相对位置的嵌入加入了自注意力的计算中，目前我先尝试一下之前自己的想法，就是简单的将时间步进行放缩，然后使用线性层进行简单映射到embedding中，然后和SASRec的处理一样加入到输入中。
但是通过简单实验发现似乎效果和没有加入这个embedding似乎差别不大？但是其实通过查看线性层的权重发现，实际上这个线性层并非想象中的没有作用，输出的值得范围还是很正常的，但是现在又有了另一个猜想，就是两个位置嵌入的直接加和会不会就间接导致物体嵌入被掩盖了，需要找方法进行验证。


### 2025.5.14
看了一些论文，简单的找到了一些论文里面的分析方法用于实践：
1. 分析用户表示之间的余弦相似度
2. 对于embedding的表示分析attention的average weights，但是这个似乎一般都是用于结论分析的时候使用的

那么对于之前的改进需要进行分析一下：
1. 对于物品tag的嵌入，改进的假设是tag或许可以反映出一个用户的偏好，那么加入tag之后应该会造成的影响有：1、同类型电影的表示相似度应该增加（当然也需要验证一下没加之前是不是本来就很高了）。2、不同类型电影表示相似度应该降低。3、用户的表示应该对于他看的比较多的电影类型更加敏感。
2. 对于时间步嵌入，改进的假设是绝对时间可以更为精确的反应用户交互的频率，那么加入的效果预期是：1、因为每个用户的交互频率的不同，加入时间步之后会使得不同用户之间的相似度降低。2、加入时间步之后注意力的权重应该在相近时间步附近热力图聚集。

今天就主要完成了对于物品tag嵌入的一些实验和对原模型引入了频域的改进，首先下面是一些实验结果：
1. 实验一：验证增加tag嵌入后同类型电影表示相似度应增加，不同类型应减少
|   模型  | 相同tag物品对之间相似度 | 随机物品对之间的相似度   |
|  :----: |  :----:  |   :----:    |
|原始模型 |  0.23608 |    0.12124 |
|优化模型 |  0.23839 |    0.12175 |

|   嵌入  | 相同tag物品对之间相似度 | 随机物品对之间的相似度   |
|  :----: |  :----:  |   :----:    |
|优化模型仅id嵌入 |  0.05814 |   0.04315 |
|优化模型仅tag嵌入 |  0.76330 |  0.25519 |
...
分析：1、优化后并没有达到预期效果。2、tag嵌入有成功学习到预期性质，但是id嵌入的占比高于tag嵌入，导致tag嵌入的融合信息作用不大，没有得到很好的利用，需要改进融合方式。3、根据对tag的统计发现存在部分高频tag，可能导致本来并不相关的物品之间的相似度被提升了，需要进一步验证。4、只是用id嵌入实际上是有潜力通过数据直接学习到item之间的相似关系的。

2. 实验二：验证增加tag嵌入后用户对于历史交互多的tag类型将更为敏感
定义一：如果一个用户历史交互类型统计中对于某个类型交互次数占比超过0.3，则认为该用户为该类型高频用户。
定义二：高频类型命中率（Dominant Tag Hit Rate, DTHR）：计算推荐列表中属于用户高频类型物品的占比
定义三：定义权重型DTRG（Weighted DTRG, WDTRG）：计算推荐列表中高频类型tag占比与所有tag占比之和的比值
|   模型  |  DTHR |  WDTRG |
|  :----: |  :----:  |   :----:    |
|原始模型 |  0.6046 |    0.3198 |
|优化模型 |  0.4412 |    0.2272 |
分析：可以看出优化模型完全不符合预期，说明优化模型并没有成功学习到用户对于历史交互多的tag类型将更为敏感的性质。
...
| Contrastive learning(random)(mean) |  0.8488  |   0.6180    |   2.5s/epoch    |
| with genre embedding |  0.7553  |   0.5224    |   5.0s/epoch    |
| with time stamp |  0.8435  |   0.6167    |   1.8s/epoch    |
| with frequency |  0.8212  |   0.5816    |   2.1s/epoch    |